...
namespace OrdinaryMagicianItems
{
    public class AdvancedDualWieldSynergyProcessor : MonoBehaviour
    {
        ...
        {
            return m_gun && m_gun.CurrentOwner && m_cachedPlayer && m_cachedPlayer.inventory.DualWielding && m_cachedPlayer.HasPickupID(PartnerGunID) && (!(m_cachedPlayer.CurrentGun != m_gun) || 
                m_cachedPlayer.CurrentGun.PickupObjectId == PartnerGunID) && (!(m_cachedPlayer.CurrentSecondaryGun != m_gun) || m_cachedPlayer.CurrentSecondaryGun.PickupObjectId == PartnerGunID);
        }
